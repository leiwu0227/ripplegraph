...
export function resolveDispatcherEntry(workflowRoot) {
    ...
    if (manifest.entryGraph && manifest.entryGraph !== dispatcher.id) {
        throw new RipplegraphError('E_ENTRY_GRAPH_MISMATCH', `workspace entryGraph ${manifest.entryGraph} does not match registered dispatcher ${dispatcher.id}`);
    }
    return dispatcher;
}
