...
export function collectUnsupportedSchemaKeywords(schema: JsonSchema, path: Array<string | number> = []): SchemaKeywordIssue[] {
  const issues: SchemaKeywordIssue[] = [];
  ...
  if (isSchemaObject(schema.items)) {
    issues.push(...collectUnsupportedSchemaKeywords(schema.items as JsonSchema, [...path, 'items']));
  }
  if ('oneOf' in schema && (!Array.isArray(schema.oneOf) || schema.oneOf.some((childSchema) => !isSchemaObject(childSchema)))) {
    ...
    schema.oneOf.forEach((childSchema, index) => {
      issues.push(...collectUnsupportedSchemaKeywords(childSchema as JsonSchema, [...path, 'oneOf', index]));
    });
  }
  return issues;
}

function unsupportedValue(path: Array<string | number>): SchemaKeywordIssue {
