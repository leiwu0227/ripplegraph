...
Legacy `knowledge/architecture/` files migrate to `knowledge/codebase/` as
revisable implementation knowledge; they are never promoted into protected
architecture. Update preflights every destination before mutation, moves bytes
unchanged, deduplicates only identical files, and reports all outcomes. Any
different destination blocks the entire migration without partial moves. After
success, the disposable knowledge index is invalidated and rebuilt on the next
search. Update also recreates the empty `project_notes/architecture/legacy/`
runtime layout without seeding protected content.
