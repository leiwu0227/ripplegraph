...
Mission and standalone Assignment completion preserves the compact activity
summary, then removes the terminal run and its owned Attempt records before
publishing the completion commit. Standalone Assignment delivery uses an exact
owned-path manifest and never broadly stages unrelated repository or installed
workflow changes.
...
`specdev publish` is the separate lane-independent micro-action for exact
user-requested eligible `project_notes/**` changes. It binds destination bytes,
baselines, source identity, archive/removal operations, and the intended Git
boundary; then rechecks, applies recoverably, writes a monotonic `P…` receipt,
and creates an exact-path commit without changing Direct/Discussion/Assignment/
Mission phase. It never bypasses `knowledge curate`, big-picture approval, or
the protected-architecture guide.

## Protected architecture

Active notes under `project_notes/architecture/` are exact-content,
user-approved project authority and outrank `knowledge/codebase/`. The
`project_notes/architecture/legacy/` archive is history, never binding.
Ordinary Brainstorms only read relevant active notes and surface conflicts; they
do not publish notes. For an explicit architecture-note request or a detected
conflict, read `_guides/architecture_guide.md` before writing. Implementation
and review also load that guide when an approved `proposed` note belongs to the
current work, or when evidence indicates an `implemented` note has drifted.
...
- **Discussion:** a concurrent code-read-only RippleGraph callable that owns a
  bounded recursive text bundle under its `brainstorm/**`, keeps proposal/design
  as canonical syntheses, and may later be promoted to fresh work.
