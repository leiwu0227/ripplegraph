...
`validators`; gates can also declare `interaction`. State responses expose the
active node's metadata so a host CLI can render prompts, enforce user-turn
pauses, offer side-channel actions, and resolve validators or commands.
